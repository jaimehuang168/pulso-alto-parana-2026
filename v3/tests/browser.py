"""Chromium/WebKit: actual browser UI, encrypted storage, and optional native local stack.
Runs only against local test URLs; never imports production cookies or credentials.
"""
import json, os, pathlib, threading, http.server, socketserver, time, base64, contextlib
...
with sync_playwright() as play:
  try:
    for engine in ['chromium','webkit']:
      ...
      check(engine+' writes ciphertext, not plaintext answers',lambda: assert_true(page.evaluate("async()=>{const v=qaVault;const row=await v.get('entries',v.prefix+'|response:'+qaEvent.id);return !!row.cipher&&!JSON.stringify(row).includes('candidate_id')&&(await v.entries()).length===1;}")))
      ...
      # Launch actual App with real SQL interpreter, synthetic accounts only.
      ...
      for width in [320,390,430]:page.set_viewport_size({'width':width,'height':844});check(engine+f' capture fits {width}px',lambda:no_overflow(page))
