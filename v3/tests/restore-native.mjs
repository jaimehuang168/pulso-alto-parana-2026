...
const original=new pg.Client({connectionString:x.dbUrl});await original.connect();let restored,createdTarget=false;const results=[];
...
try{
 ...
 await original.query('create database '+name+' template template0');createdTarget=true;u.pathname='/'+name;restored=new pg.Client({connectionString:u.toString()});await restored.connect();
 assert.equal((await restored.query('select current_database() d')).rows[0].d,name);
 // Only the brand-new disposable target's empty default schema is removed.
 // Do not pg_restore --clean into empty tables: DROP POLICY still requires its table.
 await restored.query('drop schema public;create schema extensions;create extension "uuid-ossp" with schema extensions;create extension pgcrypto with schema extensions;');
 docker(['pg_restore','-U','postgres','-d',name,'--exit-on-error','--no-owner','--no-acl',dump]);
 ...
 if(restored)await restored.end();if(createdTarget)await original.query('drop database '+name+' with (force)').catch(()=>{});docker(['rm','-f',dump]);await original.end();
