...
export function demoSource(){
 ...
 const load=async(audience='codes')=>{if(offline)throw new Error('Network disconnected (simulación)');audience=audience==='auto'?'codes':audience;seq++;
  ...
  const now=new Date().toISOString();return{schema:'pulso-live-1',audience,private:audience!=='released',can_internal:true,server_time:now,valid_until:new Date(Date.parse(now)+15000).toISOString(),poll_ms:5000,timezone:'America/Asuncion',election_date:'2026-10-04',
