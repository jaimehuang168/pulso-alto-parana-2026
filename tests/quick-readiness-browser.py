...
with sync_playwright() as p:
 ...
 check('Actual Chromium secure-context WebCrypto and isolated IndexedDB checks succeed',page.locator('#results article.pass').count()==4)
 ...
 check('Readonly login workflow runs all expected mock API checks',page.locator('#results article.pass').count()==12)
 ...
 for width in [320,390,428,768,1920]:
 ...
 check('Structured storage diagnostics preserve successful stages and separate cleanup',j['schema']=='pulso-pruebas-2' and j['storage_diagnostic']['readwrite_status']=='pass' and j['storage_diagnostic']['cleanup_status']=='pass' and len(j['storage_diagnostic']['stages'])==6)
 ...
 check('Storage-only retest preserves previous login check results',page.locator('#results article.pass').count()==12)
